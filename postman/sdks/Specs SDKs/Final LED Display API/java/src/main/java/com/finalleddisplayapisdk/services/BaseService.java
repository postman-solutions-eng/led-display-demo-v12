...
public class BaseService {
  ...
  protected FinalLedDisplayApiSdkConfig config;
   ...
   */
  public BaseService(OkHttpClient httpClient, FinalLedDisplayApiSdkConfig config) {
  ...
  /**
   * Resolves the API key auth config from the configuration hierarchy.
   *
   * @param resolvedConfig The merged request configuration (may be null)
   * @return The resolved API key auth config
   */
  protected ApiKeyAuthConfig resolveApiKeyAuthConfig(RequestConfig resolvedConfig) {
    if (resolvedConfig != null && resolvedConfig.getApiKeyAuthConfig() != null) {
      return resolvedConfig.getApiKeyAuthConfig();
    }
    return this.config.getApiKeyAuthConfig();
  }
